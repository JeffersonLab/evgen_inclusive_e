...
int FileManager::LoadInputData(const char *inpath,inputParameters_t &par){
   ...
   for(int i=0;i<NROW;i++){
	 ...
	 continue; 
      ...
      if( data[0].compare("RL")==0                ) par.RL                 = std::atof( data[1].c_str() );
      if( data[0].compare("RLb")==0               ) par.RLb               = std::atof( data[1].c_str() );
      if( data[0].compare("RLa")==0               ) par.RLa               = std::atof( data[1].c_str() );
      if( data[0].compare("ThreshType")==0        ) par.ThreshType        = std::atof( data[1].c_str() );
      if( data[0].compare("Fit_model")==0         ) par.Fit_model         = std::atof( data[1].c_str() );
